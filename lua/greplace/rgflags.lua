...
-- concatenated text of modified buffers fed to `rg -` -- and a file-selection flag
...
local function rg_types()
    ...
    if vim.fn.executable("rg") == 1 then
        -- Blocking, and on the path of a keystroke (the first `--type`
        -- completion) as well as of a search, so it is bounded: an `rg` that
        -- does not answer leaves the types empty -- `--type` then filters no
        -- buffer, which is the same as having no types to match -- rather
        -- than wedging the editor. Cached either way, so this is paid once.
        local ok, res = pcall(function()
            return vim.system({ "rg", "--type-list" }, { text = true }):wait(5000)
        end)
...
function M.parse(fargs)
    ...
    -- the search running over the whole tree with nothing said, while every
    -- other malformed flag line is reported.
