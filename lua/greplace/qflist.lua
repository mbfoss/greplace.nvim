...
local function source_of(path, cache, bufs)
...
function M.matches(root, items)
    ...
    local cache   = {}
    local dropped = 0
    -- Built once rather than per entry: looking a path up walks the buffer
    -- list resolving every name, and a quickfix list can hold thousands.
    ...
    for _, item in ipairs(items) do
        ...
        local src   = path and lnum >= 1 and source_of(path, cache, bufs) or nil
