...
--- is out of date.
function M.close()
    -- `_bufnr` is cleared by the buffer's own delete hook, so it is normally
    -- either nil or valid; the check is for the case where the buffer went
    -- without that hook running, where deleting it again would raise.
