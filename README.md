...
`greplace://greplace-matches`. Each line is the matched line itself: plain,
editable text. The `file:line` shown in front of it is *virtual*: it is not
part of the line, it cannot be typed over, and it is what tells the plugin
...
`diff` shows what `:w` would do before it does it: every file with an edited
match, as a unified diff of its text now against its text after the write, on a
tab of its own; `q` closes it. It is worked out on copies, so nothing is
applied and no file is loaded into a buffer to find out. A file that is open
is diffed from its buffer, unsaved changes included, since that is what a write
lands on. An edit the write would skip, because its source line has changed
since the search, is listed at the top instead of in the diff.

`apply` is `:w` in the panel, from wherever you are: from the preview once it
looks right, or with the panel closed. Either one closes the preview, which is
out of date once the edits have landed.
...
Anything else that is wrong with the line, such as an unknown flag or a
...
File-selection flags are also applied to open buffers, in-process, since ripgrep sees
...
Files that are open in a buffer with unsaved changes are searched from that
text rather than from disk, so the matches are the ones a write would land on.
A location is marked with `≡` whenever its file is open in a buffer, saved or
not.
...
A match is one source line and stays one line in the panel. A line that was
added is taken out again, and lines that were joined are split back apart, so
every match has its own line back; the cursor stays where the break was, and in
Insert mode you stay in Insert mode. It is the line break that comes back, not
the text: what a change deleted along with it -- the characters a charwise
delete across the break took, say -- stays deleted, and `u` brings it back. The
space `J` inserts goes with the join, so joining and splitting again leaves the
lines as they were.
...
Reports ripgrep, the commands and the options that differ from the defaults.
ripgrep is the search itself, so its absence is an error rather than a note --
without it `:Gsearch` has nothing to run. An option name greplace does not
define is reported as a warning: `setup()` merges the table you pass wholesale,
so a misspelled one would otherwise be accepted in silence.
