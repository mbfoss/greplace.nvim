...
describe(":Gsearch / :Greplace", function()
    ...
    it("writes an edited quickfix panel back to the source lines", function()
        ...
        local bufnr = assert(panel.find_buf())
        -- In place, as an edit in the panel is: replacing the line outright
        -- reads as deleting it, which drops the match from the write.
        vim.api.nvim_buf_set_text(bufnr, 0, 0, 0, #"alpha one", { "alpha ONE" })
